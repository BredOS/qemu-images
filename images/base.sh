...
function pre() {
  ...
  arch-chroot "${MOUNT}" /usr/bin/systemd-firstboot --locale=C.UTF-8 --timezone=UTC --hostname=bredos --keymap=us
...
EOF
  ...
  # Replace GRUB_DISTRIBUTOR with BredOS
  sed -i 's/^GRUB_DISTRIBUTOR=.*/GRUB_DISTRIBUTOR=\"BredOS\"/' "${MOUNT}/etc/default/grub"
